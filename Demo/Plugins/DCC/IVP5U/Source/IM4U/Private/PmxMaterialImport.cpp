	...
	TArray<UTexture*>& textureAssetList)
{
	...
	}
	{
		...
		if (nullptr != ColorTex)
		{
			...
			Param.bOverride = true;
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION > 0
			StaticParams.EditorOnly.StaticSwitchParameters_DEPRECATED.Add(Param); //DestSeq->MarkRawDataAsModified();
#else
			StaticParams.StaticSwitchParameters.Add(Param);
#endif
	...
	// StaticSwitch
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION > 0
	if (0 < StaticParams.EditorOnly.StaticSwitchParameters_DEPRECATED.Num())
	{
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION > 0
		StaticParams.EditorOnly.StaticSwitchParameters_DEPRECATED.Add(Param);//5.2失效
#else
